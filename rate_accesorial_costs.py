...
class AccessorialCost:
    ...
    name: str                                    # e.g., "Dangerous Goods"
    rate_by: Optional[str] = None                # e.g., "Quantity/Container" (just the value)
...
def find_column_indices(row_values, min_max_row=None):
    """
    Find column indices for Lane #, Currency, Flat (MIN/MAX), p/unit, Apply if in the data header row.
    ...
    Returns:
        dict with keys: 'lane', 'currency', 'flat_min', 'flat_max', 'per_unit', 'apply_if'
    """
    indices = {
        'lane': None,
        'currency': None,
...
def extract_accessorial_costs(sheet):
    ...
    for r in range(1, min(16, max_row + 1)):
        ...
        # Only show first 6 non-None values
        preview = [v for v in row_vals[:6] if v is not None]
    ...
    while current_row <= max_row:
        ...
        if first_val and is_cost_name_row(row_values, current_row):
            ...
            while scan_row <= min(current_row + 8, max_row):
...
def main():
    ...
    rate_card_files = ["rate_accessorial.xlsx"]
